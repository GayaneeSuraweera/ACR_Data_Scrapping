...
async function ACR_webscraping() {
  ...
  try {
    await driver.get("https://acr2.apx.com/myModule/rpt/myrpt.asp?r=206");
    while (true) {
      ...
      for (let i = 5; i < rows.length - 4; i++) {
        try {
          ...
          const status_effective = await cells[0].getText();
          const account_holder = await cells[1].getText();
          const quantity_of_credits = await cells[2].getText();
          const retirement_reason = await cells[3].getText();
          const retirement_reason_details = await cells[4].getText();
          const credit_serial_numbers = await cells[5].getText();
          const vintage = await cells[6].getText();
          const date_issued = await cells[7].getText();
          const verified_removal = await cells[8].getText();
          const corsia_eligible = await cells[9].getText();
          const ccp_approved = await cells[10].getText();
          const arb_eligible = await cells[11].getText();
          const ecology_eligible = await cells[12].getText();
          const sustainable_development_goal = await cells[13].getText();
          const project_id = await cells[14].getText();
          const projectId = await getProjectId(await cells[14].getText());
          const project_name = await cells[15].getText();
          const project_site_country = await cells[16].getText();
          const project_site_location = await cells[17].getText();
          const project_site_state = await cells[18].getText();
          const project_type = await cells[19].getText();
          const project_methodology_protocol = await cells[20].getText();
          const methodology_protocol_version = await cells[21].getText();
          ...
          await saveJSONToFile(
            ...
            `./retiredCreditMain/${projectId}.json`
          );
        } catch (error) {
